...
const DataBerhenti = () => {
  ...
  const generateData = async () => {
    // setLoading(true);
    try {
      const res = await KelasService.getSiswaPagingBerhenti({
        page,
        size: rowsPerPage,
        tahun,
      });
  ...
  const fetchFilteredData = async (keyword) => {
    // setLoading(true);
    try {
      const response = await KelasService.searchDataSiswaBerhenti({
        keyword,
        page,
        size: rowsPerPage,
        tahun,
  ...
  const handleUndo = async (data) => {
    setLoading(true);
    try {
      const response = await KelasService.undoBerhenti(data);
      console.log("RESPONSE DATA UNDO SISWA:", response);
      if (response.success) {
        setToastMessage("Siswa berhasil diaktifkan kembali");
      ...
      >
        ...
        >
          <Box>
            ...
            >
              Kelola data arsip siswa berhenti
            </Typography>
          </Box>
        </Box>
        <Divider sx={{ mb: 2 }} />
        ...
        >
          <FormControl margin="normal">
            <InputLabel id="tahun-label">Pilih Tahun Berhenti</InputLabel>
            <Select
              labelId="tahun-label"
              label="Pilih Tahun Berhenti"
              value={tahun}
              onChange={(e) => {
              ...
              {data.length > 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={9} align="center">
                    Masih belum ada data siswa berhenti
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
        ...
        <ConfirmModal
          ...
          question="Apakah anda yakin akan mengaktifkan siswa ini kembali"
          icon={
            <PrivacyTipTwoToneIcon sx={{ fontSize: 48, color: "#f44336" }} />
          }
          useText={true}
          textCancel="Ngga jadi deh!"
          textSubmit="Ok, aktifin aja"
          onConfirm={() => {
            setUndo(false);
